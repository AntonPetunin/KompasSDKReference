...
<div class="interface" style="margin: 5px 30%;">IDocuments</div>
</div>
...
Интерфейс `IDocuments` наследует методы [`IKompasCollection`](docs/Приложение KompasAPI/Документы/Документ 2D/Вид/Коллекции объектов/IKompasCollection.md) и расширяет его функциональность специфичными методами для работы с документами. Через этот интерфейс можно работать со всеми типами документов КОМПАС: чертежами (2D), моделями (3D), спецификациями, фрагментами и другими.
...
2. **Закрытие документов** - После работы с документом необходимо корректно закрыть его с помощью метода [`Close()`](interface_page_files/IKompasDocument.md), указав параметр сохранения изменений.
...
1. **Из приложения:**
   - [`IApplication::GetDocuments()`](docs/Приложение KompasAPI/IApplication.md) - основной способ получения коллекции документов
...
[Группа методов](#группа-5-дополнительные-методы) | [К оглавлению](#методы-интерфейса)
